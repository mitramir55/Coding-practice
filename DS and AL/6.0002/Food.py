...
sort_key_funct = lambda instance: instance.value
...
def greedy(items, sort_key_funct, max_cost=60):
    ...
    for i in range(len(items)):
        if total_cost + items[i].get_cost() < max_cost:
...
def test_greedy(items, sort_key_funct, max_cost=60):
    final_list, total_value = greedy(items, sort_key_funct, max_cost=60)
    print('Total value = ', total_value)
    for i in final_list: print('item: ', i.name)

test_greedy(items=menu, sort_key_funct= lambda x:x.get_cost(), max_cost=60)
